...
export class Tasks {
    ...
    async updateOrder(taskId, order) {
      const response = await this._client.post(`/tasks/${taskId}/updateorder`, {order: order});
      return response.data();
    }
    ...
    async settings (enabled, type) {
      const response = await this._client.post(`/tasks/settings`, {}, {enabled: enabled, type: type})
      const data = response.data()
    ...
    userPrivateTasksSubtasks (taskId) {
      return new UserPrivateTasksSubtasks(this._client, taskId)
    }
